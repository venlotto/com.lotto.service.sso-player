-- Consolidated baseline for com.lotto.service.sso-player.
--
-- v1/v2/v3 were squashed into this single migration: the service is not
-- yet in production, so there is no deployed database whose history has
-- to be preserved. Player usernames are phone numbers in the canonical
-- 0-prefixed local form (0412…) from the start, which is why the v3 data
-- fix-up is not carried over — a fresh database has nothing to canonicalize.
--
-- Generated from the dev1 schema so a new environment comes up identical
-- to the one that has been exercised end to end.
...
-- CreateIndex
CREATE UNIQUE INDEX "refresh_tokens_token_id_key" ON "refresh_tokens"("token_id" ASC);
...
-- CreateIndex
CREATE UNIQUE INDEX "roles_permissions_role_id_permission_id_key" ON "roles_permissions"("role_id" ASC, "permission_id" ASC);

-- CreateIndex
CREATE UNIQUE INDEX "users_username_key" ON "users"("username" ASC);

-- CreateIndex
CREATE UNIQUE INDEX "users_roles_user_id_role_id_key" ON "users_roles"("user_id" ASC, "role_id" ASC);
